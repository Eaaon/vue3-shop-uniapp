<template>
  <view class="content" >
    <view class="bar-header" :style="{backgroundImage:`url(${indexBackgroundImage})`}">
      ...
      <view class="flex header" style="height:60rpx" v-else>
        <view>
          <image
            src="../../static/svg/location.svg"
            class="location-img"
    ...
    <uni-swiper-dot class="uni-swiper-dot-box"  :info="state.info" :current="state.current" :mode="state.mode" 
      :dots-styles="state.dotsStyles" field="content">
      <swiper class="swiper-box" @change="change" :current="state.swiperDotIndex" circular>
        <swiper-item v-for="(item, index) in state.info" :key="index" @click=clickItem(item)>

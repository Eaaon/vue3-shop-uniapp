...
<script setup>
  ...
  import throttle from '@/utils/helper/throttle.js';
  // import { showMenuTools, closeMenuTools } from '@/shop/hooks/useModal';

  const sys_statusBar = shop.$platform.device.statusBarHeight;
  const sys_navBar = shop.$platform.navbar;
  const capsuleStyle = {
    width: shop.$platform.capsule.width + 'px',
    height: shop.$platform.capsule.height + 'px',
  };
  ...
  // const hasHistory = shop.$router.hasHistory();

  function onClickLeft() {
    // if (hasHistory) {
    //   shop.$router.back();
    // } else {
    //   shop.$router.go('/pages/index/index');
    // }
    // emits('clickLeft');
  }
  ...
  onPageScroll((e) => {
    state.tabOpacityVal = e.scrollTop > shop.$platform.navbar ? 1 : e.scrollTop * 0.01;

  ...
  <view class="ui-fixed">
      ...
      :style="[
        {
          ...
          bottom: insetHeight,
          zIndex: index + shop.$zIndex.navbar,
        },
    ...
    >
      <view
        ...
        @tap="toTop"
        :style="[{ zIndex: index + shop.$zIndex.navbar }]"
      >
        <slot></slot>
        <view
      ...
      <view
        ...
        :class="[ui, bg]"
        :style="[
          { zIndex: index + shop.$zIndex.navbar - 1 },
          bgStyles,
...
<script setup>
  ...
  const { safeAreaInsets } = shop.$platform.device;

  const vm = getCurrentInstance();

  const uuid = shop.$helper.guid();
  ...
  onPageScroll((e) => {
    let top = e.scrollTop;
    state.scrollTop = top;
    state.opacityVal = top > shop.$platform.navbar ? 1 : top * 0.01;
  });
    ...
    if (unref(props.bottom)) {
      state.fixed =
        ...
        state.content.bottom -
          shop.$platform.device.windowHeight +
      ...
      state.fixed =
        value >=
        state.content.top -
          (unref(props.noNav) ? unref(props.val) : unref(props.val) + shop.$platform.navbar);
    }
  };

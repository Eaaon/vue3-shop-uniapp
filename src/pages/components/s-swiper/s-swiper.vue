<template>
  <uni-swiper-dot :info="list" :current="state.current" :mode="state.mode" :dots-styles="state.dotsStyles"
    field="content">
    <swiper class="mt-3 h-300" @change="change" :current="state.swiperDotIndex" circular autoplay>
      <swiper-item v-for="(item, index) in list" :key="index" @click="clickItem(item)">
